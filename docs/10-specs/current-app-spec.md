...
対象: `apps/local/`（Mac常駐版・**稼働中**） / 現行バージョン: v1.38.2（画面右上に表示）
最終更新: 2026-07-26
...
| `number_reading.py` | 算用数字の中国語読み（§7.2） |
| `pinyin_fixes.py` | **pypinyin の読み間違いの補正辞書**（§7.3）。誤りを見つけたら1行足す |
...
| 43.1 | **多音字（`to_pinyin_pairs`）** | 漢字は**連なりごと**に pypinyin へ渡す。1文字ずつ渡すと語彙辞書が効かず誤る（行业 → xíng yè）。§7.3 | **移植（重要）** |
| 43.2 | **算用数字の中国語読み** | `number_reading.py`。数字を**中国語の数詞として**読む（15000 → 一万五千）。文脈で1桁ずつに切り替える（年号・型番・証券コード）。§7.2 | **移植（重要）** |
...
### 7.2 算用数字の中国語読み（`number_reading.py`）

**以前は数字を1桁ずつ読んでいたが、数量・日付では誤りだった**（15000 → yī wǔ líng líng líng）。
そのまま覚えると誤った中国語になるため、**中国語の数詞として読む**ように改めた。

**中国語の数詞は位取りなので、桁ごとに音節を割り当てられる。**

| 数 | 割り当て |
|---|---|
| 250 | 2→`èr bǎi` / 5→`wǔ shí` / 0→（なし） |
| 15 | 1→`shí` / 5→`wǔ`（**一十五とは読まない**） |
| 105 | 1→`yī bǎi` / 0→`líng` / 5→`wǔ` |
| 15000 | 1→`yī wàn` / 5→`wǔ qiān` / 0,0,0→（なし） |

これにより **`pairs` の長さが原文の文字数と一致したまま**になる。
記事モードの1文字単位の失敗記録（`ci`）がこの並びに依存しているため、**崩してはいけない**。

**文脈による切り替え**（記事29本・287件の実データから決定）

| 直後／直前 | 読み | 実データ |
|---|---|---|
| 直後が `年` | **1桁ずつ**（2026 → `èr líng èr liù`） | 54件 |
| 直後が `%` | **`bǎi fēn zhī` を数の前に置く**（99.99% → 百分之九十九点九九） | 29件 |
| 直後が `）` で5桁以上 | **1桁ずつ**（証券コード 688160） | 4件 |
| 直前がラテン文字 | **1桁ずつ**（型番 GR00T / G2 / HR2026） | 5件 |
| それ以外 | **数として読む**（月日・台・万・亿・个…） | 195件 |

小数は「整数部を数として読む → `diǎn` → 小数部は1桁ずつ」（1.5万 → `yī diǎn wǔ wàn`）。

> **`%` は語順が反転する。** 中国語では「百分之」が数の**前**に来るため、
> 読み上げ順が正しくなるよう**数の先頭の桁**に付ける。末尾の `%` 自体は読まない。

> **落とし穴**：Python では `"" in "%％"` が **True** になる。文末の数字が
> パーセント扱いされるため、`nxt` が空でないことを先に確かめること（テスト N-50）。

### 7.3 多音字と変調（`to_pinyin_pairs`）

**以前は漢字を1文字ずつ pypinyin に渡していたため、語彙の文脈が失われ多音字を誤っていた。**
漢字の**連なりごと**に渡すよう改めた（`HAN_RUN` で切り出す）。

| 語 | 誤（1文字ずつ） | 正（連なりごと） |
|---|---|---|
| 行业 | xíng yè | **háng yè** |
| 银行 | yín xíng | **yín háng** |
| 供应链 | gōng yīng liàn | **gōng yìng liàn** |
| 重新 | zhòng xīn | **chóng xīn** |
| 长期 | zhǎng qī | **cháng qī** |
| 一个 / 不是 | yī gè / bù shì | **yí gè / bú shì**（変調） |

**記事29本・漢字13,305字のうち219字（1.6%）が変わる。** 最多は 应（42件）・为（40件）・一（27件）。

> **返り値の長さは必ず原文の文字数と一致させる。** 想定外の分割が返った場合は
> 1文字ずつに退避する（`ci` を壊さないため）。

**記号も読む**（`SYMBOLS`）。`±0.1毫米` → **zhèng fù** líng diǎn yī háo mǐ。
...
| `°` | dù（度） | 常に |
| `≈` | yuē děng yú（约等于） | 常に |
| `×` | chéng（乘） | **数字に挟まれているときだけ** |
| `~` `〜` `～` | dào（到） | **数字に挟まれているときだけ** |

> `×` や `~` を無条件に読むと「视觉-语言-动作」のような**ラテン文脈の記号**まで読んでしまう。
> `-` `+` は実データでは型番・語の連結（`Pre-A+`, `ISO/TS`）にしか出ないため、読まない。

#### 補正辞書（`pinyin_fixes.py`）

**連なりごとに渡しても、pypinyin の語彙辞書に無い語は誤る。** その分を補正辞書で直す。
pypinyin 公式の `load_phrases_dict()` を使うので、後処理の書き換えは不要。

| 語 | pypinyin | 正 |
|---|---|---|
| 视为 | shì **wèi** | shì **wéi**（〜とみなす） |
| 列为 | liè **wèi** | liè **wéi**（〜に列せられる） |
| 身为 | shēn **wèi** | shēn **wéi**（〜の身として） |
| 华为 | huá **wèi** | huá **wéi**（社名） |

> **誤りを見つけたら `FIXES` に1行足すだけ**で直る。`python3 -m pinyin_fixes` で自己チェック
> （字数と要素数の一致・実際にその読みになるか）ができる。
>
> **書いてよいのは「その語なら常にこう読む」ものだけ。** 文脈で変わるものを押し込むと
> 別の文脈を壊す（`因为` wèi / `作为` wéi のように**語ごとに決まる**ものは可）。

> **残る限界**：辞書に無い未知の語は依然として誤りうる。気づいたら足す運用にしている。
