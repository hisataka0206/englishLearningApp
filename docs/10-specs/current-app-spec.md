...
対象: `apps/local/`（Mac常駐版・**稼働中**） / 現行バージョン: v1.38.0（画面右上に表示）
最終更新: 2026-07-26
...
> **限界**：pypinyin の語彙辞書に無い**短い断片**は今も誤る（「视为」単独 → shì wèi）。
> 文として渡せば正しくなる（「被视为人形机器人」→ shì **wéi**）。
> アプリは常に句単位で渡すため実害は小さいが、単語帳の短い語では起こりうる（テスト P-13）。
